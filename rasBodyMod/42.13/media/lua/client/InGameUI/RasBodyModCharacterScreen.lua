    ...
    if gender == "Female" then -- options for women
                   ...
                   if currentStyle ~= "None" then 
                        if shaveTable[currentStyle] then -- generate options for shaving to a special style
                             for _,v in pairs(shaveTable[currentStyle]) do
                                  option = bodyHairMenu:addOption(getText("UI_ClothingType_" .. location:toString()) .. ": " .. v.optionText , player, onShaveBodyHair, v.canBeShavedTo, location);
                        ...
                        option = bodyHairMenu:addOption(getText("UI_ClothingType_" .. location:toString()) .. ": " .. getText(shaveAllText), player, onShaveBodyHair, "None", location); -- can always be shaved to none
	                    if not hasRazor then   
		                       self:addTooltip(option, getText("UI_rasBodyMod_RequireRazor"));
                        end 
                   end
         ...
         for location,_ in pairs(rasSharedData.BodyHairLocations) do
               ...
               if currentStyle ~= "None" then 
                    ...
                    option = bodyHairMenu:addOption(getText("UI_ClothingType_" .. location:toString()) .. ": " .. getText(shaveAllText), player, onShaveBodyHair, "None", location);
                    if not hasRazor then   
	                       self:addTooltip(option, getText("UI_rasBodyMod_RequireRazor"));
                    end 
               end      
