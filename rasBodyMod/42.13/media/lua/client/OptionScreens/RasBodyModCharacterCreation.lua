...
local UIText = {} -- the ui text for the new body hair location
UIText[Regs.PubicHair] = "UI_ClothingType_RasPubicHair"
UIText[Regs.ChestHair] = "UI_ClothingType_RasChestHair"
UIText[Regs.ArmpitHair] = "UI_ClothingType_RasArmpitHair"
UIText[Regs.LegHair] = "UI_ClothingType_RasLegHair"
...
local function isClothingLocation(location)
    ...
    if rasSharedData.ModdedLocation(location) then
        return false
    end
...
local function torsoNude(desc)
    ...
    for _,loc in pairs(locations) do
        ...
        if theLocation then
            local item = desc:getWornItem(theLocation)
...
local function manageMalePrivatePart(self, desc, skinColor)      
     if not desc:isFemale() then
            if wearsExceptionalClothing(desc) then
                desc:setWornItem(Regs.MalePrivatePart, nil)
            elseif rasSharedData.PenisTable[skinColor] and rasSharedData.PenisTable[skinColor]["default"] then
                  ...
                  local pubicHair = desc:getWornItem(Regs.PubicHair) 
                  ...
                  desc:setWornItem(Regs.MalePrivatePart, item)
            end
...
local function equipBodyHair(self, desc, bodyLocation, skinColor)
     ...
     local hairItem = rasClientData.SelectedBodyHair[gender][bodyLocation:toString()] -- get default body hair item
...
local function assignRandomBodyHair(desc)
         if desc:isFemale() then -- for female               
               ...
               if n <= 25 then
                   rasClientData.SelectedBodyHair["Female"]["rasbomo:armpithair"] = "RasBodyMod.FemaleArmpit"
               else
                   rasClientData.SelectedBodyHair["Female"]["rasbomo:armpithair"] = "None"
               end
               -- legs
               n = ZombRand(101)
               if n <= 5 then
                   rasClientData.SelectedBodyHair["Female"]["rasbomo:leghair"] = "RasBodyMod.FemaleLeg"
               else
                   rasClientData.SelectedBodyHair["Female"]["rasbomo:leghair"] = "None"
               end
               -- assign pubic hair
               n = ZombRand(101)
               if n <= 55 then        
                    rasClientData.SelectedBodyHair["Female"]["rasbomo:pubichair"] = "RasBodyMod.FemalePubicNatural"
               elseif n <= 85 then
                    rasClientData.SelectedBodyHair["Female"]["rasbomo:pubichair"] = "RasBodyMod.FemalePubicTrimmed"
               elseif n <= 95 then
                   rasClientData.SelectedBodyHair["Female"]["rasbomo:pubichair"] = "RasBodyMod.FemalePubicStrip"
               else
                    rasClientData.SelectedBodyHair["Female"]["rasbomo:pubichair"] = "None" -- in this case, remove all body hair for more visual consistency
                    rasClientData.SelectedBodyHair["Female"]["rasbomo:leghair"] = "None"
                    rasClientData.SelectedBodyHair["Female"]["rasbomo:armpithair"] = "None"
               end 
         else -- for male
               ...
               local n = ZombRand(101)
               if n <= 5 then
                   rasClientData.SelectedBodyHair["Male"]["rasbomo:armpithair"] = "None"
               else
                   rasClientData.SelectedBodyHair["Male"]["rasbomo:armpithair"] = "RasBodyMod.MaleArmpit"
               end
               -- legs
               n = ZombRand(101)
               if n <= 5 then
                   rasClientData.SelectedBodyHair["Male"]["rasbomo:leghair"] = "None"
               else
                   rasClientData.SelectedBodyHair["Male"]["rasbomo:leghair"] = "RasBodyMod.MaleLeg"
               end
               -- chest
               n = ZombRand(101)
               if n <= 90 then
                   rasClientData.SelectedBodyHair["Male"]["rasbomo:chesthair"] = "RasBodyMod.MaleChest"
               else
                   rasClientData.SelectedBodyHair["Male"]["rasbomo:chesthair"] = "None"
               end   
               -- assign pubic hair
               n = ZombRand(101)
               if n <= 85 then        
                  rasClientData.SelectedBodyHair["Male"]["rasbomo:pubichair"] = "RasBodyMod.MalePubicNatural"
               elseif n <= 95 then
                  rasClientData.SelectedBodyHair["Male"]["rasbomo:pubichair"] = "RasBodyMod.MalePubicStrip"
               else
                  rasClientData.SelectedBodyHair["Male"]["rasbomo:pubichair"] = "None"
                  rasClientData.SelectedBodyHair["Male"]["rasbomo:chesthair"] = "None" -- also remove chest hair (for more visual consistency)
               end 
...
local function equipAccordingToSelectedBodyHair(self, desc, skinColor)
        ...
        if desc:isFemale() then
           for location,_ in pairs(rasSharedData.BodyHairLocations) do
                 if location ~= Regs.ChestHair then -- no chest hair for women
...
local function adjustScreen(self)
        ...
        BH_LBL_LEN = math.max(
            getTextManager():MeasureStringX(UIFont.Small, getText(UIText[Regs.ChestHair])),
            getTextManager():MeasureStringX(UIFont.Small, getText(UIText[Regs.ArmpitHair])),
            getTextManager():MeasureStringX(UIFont.Small, getText(UIText[Regs.PubicHair])),
            getTextManager():MeasureStringX(UIFont.Small, getText(UIText[Regs.LegHair]))
        )       
        ...
        if CharacterCreationMain:shouldShowAllOutfits() then -- compute max length of the clothing labels (if all clothing unlocked enabled)
            ...
            for i=1,allLocations:size() do
                ...
                if isClothingLocation(bodyLocation) then
                    local name = bodyLocation:getTranslationName()
                    if name then
            ...
            local profession = desc:getCharacterProfession():getName()
...
local function updateSelectedBodyHairCombo(self)
	...
	if self.bodyHairCombo then
		for i,combo in pairs(self.bodyHairCombo) do
			...
			if currentItem then
                local displayName = rasClientData.SelectedBodyHair[gender][combo.bodyLocation:toString()]
...
local function onBodyHairComboSelected(self, combo, bodyLocation)
	...
	rasClientData.SelectedBodyHair[gender][bodyLocation:toString()] = "None"
	
	local itemType = combo:getOptionData(combo.selected)
	if itemType then
		local item = instanceItem(itemType)
		if item then
		     rasClientData.SelectedBodyHair[gender][bodyLocation:toString()] = item:getFullType()          
	         desc:setWornItem(bodyLocation, item)			
		end  
	end
...
local function doBodyHairCombo(self, definition)
	...
	for bodyLocation, profTable in pairs(definition) do          
	    if rasSharedData.BodyHairLocations[bodyLocation] then  
		        ...
		        if not combo then
			        createBodyHairCombo(self, getText(UIText[bodyLocation]), bodyLocation);
			        combo = self.bodyHairCombo[bodyLocation];
    ...
    if desc:isFemale() then
        ...
        if item then
            ...
            elseif itemType == "Base.BoobTube" or itemType == "Base.BoobTubeSmall" then 
                desc:setWornItem(ItemBodyLocation.TANK_TOP, nil)
                desc:setWornItem(ItemBodyLocation.UNDERWEAR_TOP, nil)
                local bra = instanceItem("Base.Bra_Strapless_White")
                desc:setWornItem(ItemBodyLocation.UNDERWEAR_TOP, bra)
                self:updateSelectedClothingCombo() 
                self.avatarPanel:setSurvivorDesc(desc)
            end
        end
...
function CharacterCreationMain:initClothing(...)    
    ...
    if desc:getCharacterProfession():getName() ~= PROFESSION then
          PROFESSION = desc:getCharacterProfession():getName()
...
function CharacterCreationMain:createClothingComboDebug(bodyLocation, ...)
      ...
      local loc = ItemBodyLocation.get(ResourceLocation.of(bodyLocation))
      if isClothingLocation(loc) then -- clothing items only
    ...
    if not desc:isFemale() then
	    if selected then
            ...
            if not rasSharedData.FullBeards[beard] then -- only show stubble when not having a full beard
		         ...
		         desc:setWornItem(Regs.BeardStubble, stubble)
            end
            rasClientData.BeardStubble = 1
	    else
		    desc:setWornItem(Regs.BeardStubble, nil) 
...
function CharacterCreationMain:onBeardTypeSelected(combo, ...)
    ...
    if rasSharedData.FullBeards[beard] then
         if rasClientData.BeardStubble == 1 then
            desc:setWornItem(Regs.BeardStubble, nil) -- hide stubble when having a full beard
         end 
    elseif rasClientData.BeardStubble == 1 and desc:getWornItem(Regs.BeardStubble) == nil then
         local stubble = instanceItem("RasBodyMod.StubbleBeard_Light")
         if rasClientData.SkinColorIndex == 4 or rasClientData.SkinColorIndex == 5 then
                stubble = instanceItem("RasBodyMod.StubbleBeard_Dark")
         end
		 desc:setWornItem(Regs.BeardStubble, stubble) -- show stubble when no full beard
    end
...
function CharacterCreationMain:onShavedHairSelected(index, selected, ...)
    ...
    if selected then
         ...
         if desc:isFemale() then
		      ...
		      desc:setWornItem(Regs.HeadStubble, stubble)
         else
         ...
         desc:setWornItem(Regs.HeadStubble, nil)
    end
         ...
         for i,v in pairs(self.bodyHairCombo) do
                if locID[i] then
		            if v:getOptionData(v.selected) ~= nil then
			            savestring = savestring ..  locID[i] .. "=" .. v:getOptionData(v.selected)
...
local reverseID = {}
reverseID["RasPubicHair"] = Regs.PubicHair 
reverseID["RasChestHair"] = Regs.ChestHair
reverseID["RasArmpitHair"] = Regs.ArmpitHair
reverseID["RasLegHair"] = Regs.LegHair 
...
function CharacterCreationMain:loadOutfit(box,...)
       ...
       if desc:isFemale() then
           rasClientData.SelectedBodyHair["Female"]["rasbomo:armpithair"] = "None"
           rasClientData.SelectedBodyHair["Female"]["rasbomo:pubichair"] = "None"
           rasClientData.SelectedBodyHair["Female"]["rasbomo:leghair"] = "None"
           rasClientData.HeadStubble = 0
       else
           rasClientData.SelectedBodyHair["Male"]["rasbomo:chesthair"] = "None"
       ...
       for i,v in pairs(items) do -- load body hair and beard/head stubble
               ...
               local bodyLocation = reverseID[location[1]]
               if self.bodyHairCombo[bodyLocation] then
