...
local function mainInitFunction(player, square)    
    ...
    if isClient() then -- in MP, we can only send commands to server after the first few game ticks, so we wait a bit
        
        local initTick = nil
        local function initInMP(tick)

            if not initTick then
                initTick = tick
            end

            if tick >= initTick + 2 then -- wait 2 ticks
...
local function mainStartFunction()
    ...
    if isClient() then -- for MP   
     
        local startTick = nil -- as in the init function for MP, wait a few ticks before doing anything
        local function startInMP(tick)
            ...
            if tick >= startTick + 3 then
                startTick = nil
                Events.OnTick.Remove(startInMP) -- remove from events
                local data = player:getModData().RasBodyMod
                sendClientCommand(player, "rasBodyMod", "createPlayerOnServer", {modData = data}) -- in MP, we send a command to server to sync modData and then call the start procedure on client-side  
            end                                                                                   -- (note: in MP, modData can only be stored permanently on server)
        end
        
        Events.OnTick.Add(startInMP)   
...
local createPlayerInMP = {} -- can be accessed by other luas in client folder via require("RasBodyModCreatePlayer")

-- function for creating the player in MP, will be triggered by a command coming from server
function createPlayerInMP.create(player, modData)
    
    local data = player:getModData()
    
    data.RasBodyMod = modData -- modData coming from server (will be nil when mod added to an existing game)  
    setUpModData(player)
    initBody(player) -- equip body items to player
end
...
function createPlayerInMP.calibrate(player)
