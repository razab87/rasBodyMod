...
local manageTurnAround = {} -- can be accessed via require("ManageBodyClient/RasBodyModManageTurnAround") by other .lua files from client
               ...
               if (not player:isAiming()) and (not player:isSneaking()) then                                                               
                        ...
                        if not IS_MOVING and (getTimestampMs() - movementKey[key]["time"] < 200) and movementKey[key]["direction"] ~= PLAYER_DIR and (not movementKey[key]["cancel"]) 
                           and isValid(player) then 
                                  ...
                                  if data.RasBodyMod then
                                      if data.RasBodyMod.PlayerMode ~= "turn" then   
                                          data.RasBodyMod.PlayerMode = "turn"                        
                                          util.equipNow(player) -- will actually equip the model 
