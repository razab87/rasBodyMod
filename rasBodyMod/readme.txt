...
- 1.5.9: added compatibility for singleplayer with the most recent game update to 42.13; multiplayer is not yet supported and requires some more work from my side

- 1.5.10: fixed some non-english translations broken by last update; fixed some glitchy visuals when male characters sit-on-ground and get up via clicking smth in right-click menu (without wearing clothes)

- 1.5.11: fixed error occuring when male character starts game while wearing certain kinds of backpacks in B42.13; fixed graphical enhancements for male characters sometimes not applied

- 1.5.12: fixed missing translations for the in-game shave-menu; fixed in-game avatar sometimes not showing the same dirt/blood pattern as the in-game character model; removed some experimental multiplayer coder (no impact for singleplayer mode but makes the code cleaner)

- 1.6: added MP support for B42

- 1.6.1 (current): fixed characters do not have their new body items equipped after dying in B42 multiplayer and choosing "continue with new character"
