...
- 1.5.12 (current): fixed missing translations for the in-game shave-menu; fixed in-game avatar sometimes not showing the same dirt/blood pattern as the in-game character model; removed some experimental multiplayer coder (no impact for singleplayer mode but makes the code cleaner); made the mod not selectable when in MP (since it was not meant to be functional in MP); removed a procedure for graphical enhancement which seems to be obsolete in recent game version
