...
function ISFitnessAction.update(self, ...) 
	...
	if self.exercise == "situp" and not self.character:isFemale() then
	      ...
	      if not ADDED_EQUIPDEFAULTPENIS_TO_EVENTS and 
             (self.character:pressedMovement(true) or self.character:getMoodles():getMoodleLevel(MoodleType.Endurance) > ISFitnessUI.enduranceLevelTreshold 
              or getGameTime():getCalender():getTimeInMillis() > self.endMS) then 
                       ...
                       local function equipDefaultPenis(tick)
                               ...
                               if getTimestampMs() >= startTime + delay then   -- equip the model after a short delay for better visuals 
                                      ...
                                      manageBody.ManageMalePrivatePart(self.character, false)
...
-- when fitness is over due to pressing ESC or using a right-click action, we re-equip the default penis here
...
function ISFitnessUI.onClick(self, button, ...) 
   ...
   if button.internal == "CANCEL" then
            ...
            if not player:isFemale() and data.RasBodyMod.PrepareForSitups and not ADDED_EQUIPDEFAULTPENIS_TO_EVENTS then
