...
function shaveBodyHairAction:isValid()
     ...
     if self.item and self.item:getFullType() == "Base.Razor" and (not self.item:isBroken()) and self.item:getRemainingUses() >= 1 then
...
function shaveBodyHairAction:perform()
    ...
    data["DaysTilGrow"][self.bodyLocation] = 3 + ZombRand(2) -- reset counter for body hair growth 
   	
	data[self.bodyLocation] = self.choice 
