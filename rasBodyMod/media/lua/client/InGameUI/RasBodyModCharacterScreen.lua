...
local function predicateUsableRazor(item)
	return ( (not item:isBroken()) and item:getRemainingUses() >= 1)
end
...
function ISCharacterScreen.render(self, ...)     
    ...
    -- find the y-positions for hair and beard ui (will be stored in finalY) so that we find the correct position for body hair ui
    -- same computations as in vanilla:
    local smallFontHgt = getTextManager():getFontFromEnum(UIFont.Small):getLineHeight()
    local z = 25 + FONT_HGT_MEDIUM + 10 + smallFontHgt + smallFontHgt
    ...
    finalY = finalY + 20 -- postion for hair style button
    finalY = finalY + smallFontHgt + 4 -- position for beard or body hair (depending on male/female)
    if not self.char:isFemale() then -- in case we are male, we need additional space for the beard options
             finalY = finalY + smallFontHgt + 4
    end
      
             
    -- adjust the x-position for the ui elements
	...
	self.hairButton:setX(buttonsX) -- must adjust the x-poisitons
	self.beardButton:setX(buttonsX)
       
	
	-- draw ui elements for body hair shaving	
	self:drawTextRight(getText("UI_rasBodyMod_BodyHair"), self.xOffset, finalY, 1,1,1,1, UIFont.Small);
    self:drawText(summary, self.xOffset + 10, finalY, 1,1,1,0.5, UIFont.Small);
	self.bodyHairButton:setVisible(true);
	self.bodyHairButton:setX(buttonsX);
	self.bodyHairButton:setY(finalY);
	...
	self.literatureButton:setY(finalY + 4) -- change position of "literature" button a bit so that we have enough space for the body-hair-shaving option
...
function ISContextMenu.addOption(self, name, target, onSelect, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10, ...)
     ...
     if CALLED_BY_BEARD_MENU then -- only true when addOption() is called by ISCharacterScreen.beardMenu() (see below)
            ...
            if scissors or razor then -- create option for shaving beard with razor or scissors
...
function ISCharacterScreen.beardMenu(self, button, ...)
     ...
     local modInfo = getModInfoByID("improvedhairmenu") -- disable beard stubble shaving options when mod "Improved Hair Menu" is active; this mod overwrites critical vanilla functions, so we disable
     if modInfo and isModActive(modInfo) then           -- to avoid bugs (means missing functionality for players but bugs are even worse; it is just as vanilla then)
     ...
     local currentBeard = getBeardStylesInstance():FindStyle(self.char:getHumanVisual():getBeardModel())
     if (not improvedHairMenuActive) and (not isDebugEnabled()) and (currentBeard == nil or currentBeard:getLevel() <= 0) then -- in this case, shaving options are not created by beardMenu() so we create them here
          local data = self.char:getModData().RasBodyMod          
